...
<form action="/submitMembership.php" method="POST" id="memForm" class="memForm" enctype="multipart/form-data">
  <!--Name-->
  <div id="namesdiv">
...
<input class="input" type="date" name="dob" placeholder="dob*" required/><br/>
  ...
  <button class="input" type="button" name="submitNoOne" value="Submit">Pay</button>
</form>
...
<script>
  const sku = "membership-order-001"; // Fixed SKU for customization fee
      ...
      cardButton.addEventListener('click', async function () {
        try {
          ...
          document.getElementById('payment-status').textContent = result.success
            ...
            if(result.success){
    document.querySelector('#memForm').submit();
}
        } catch (err) {
